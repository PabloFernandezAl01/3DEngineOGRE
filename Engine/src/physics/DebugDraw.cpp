...
namespace Physics {
	...
	DebugDraw::DebugDraw() 
	{
		renderer = Renderer::RendererManager::Instance();
	...
	void DebugDraw::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
	{
		...
		SDL_SetRenderDrawColor(SDLRenderer, color.r * 255, color.g * 255, color.b * 255, 255);
		...
		for (int i = 1; i < vertexCount; i++) {
			...
			renderer->AdjustRectToCamera(&ax, &ay, nullptr, nullptr, {1, 1});
			renderer->AdjustRectToCamera(&bx, &by, nullptr, nullptr, {1, 1});

			SDL_RenderDrawLine(SDLRenderer, ax, ay, bx, by);
		}
	...
	void DebugDraw::DrawPoint(const b2Vec2& p, float size, const b2Color& color) 
	{
		...
		renderer->AdjustRectToCamera(&rect.x, &rect.y, &rect.w, &rect.h, {1, 1});
