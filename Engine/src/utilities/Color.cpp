...
namespace Utilities {
	...
	Color Color::Lerp(const Color& startColor, const Color& endColor, float t) {
		int r = static_cast<int>(std::round(startColor.r * (1.0f - t) + endColor.r * t));
		int g = static_cast<int>(std::round(startColor.g * (1.0f - t) + endColor.g * t));
		int b = static_cast<int>(std::round(startColor.b * (1.0f - t) + endColor.b * t));
		return Color(r, g, b);
	}
